...
text_df = tibble(line = 1: nrow(dat), text = as.character(dat$review_body))
total_df = text_df %>% 
  unnest_tokens(word, text) %>% 
  anti_join(stop_words) %>% 
  inner_join(nrc) %>% 
  group_by(line) %>% 
  summarise(total_emotion = n())
```

```{r}
dat = dat %>% 
  mutate(line = 1:nrow(dat)) %>% 
  left_join(total_df, by = "line") %>% 
  na.omit() %>% 
  mutate(star_rating_square = star_rating^2) %>% 
  dplyr::select(review_date, star_rating_square, group, total_emotion, helpful_votes, total_votes)
...
for(i in 1:nrow(dat))
{
  if(dat$total_votes[i] != 0 & dat$helpful_votes[i] / dat$total_votes[i] >= 0.5)
  {
    dat$helpful[i] = 1
  }
}
...
dat_original$helpful %>% 
  table()

# 临时

dat_original %>%
  ggplot(aes(x = star_rating_square, y = helpful)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm")

dat_original$total_emotion = scale(dat_original$total_emotion)
dat_original$star_rating_square = scale(dat_original$star_rating_square)
```
...
# --- prepare
dat = dat_original %>% 
  dplyr::select(star_rating_square, total_emotion, helpful, group)
```
...
```{r echo=FALSE}
flds <- createFolds(dat$star_rating_square, k = 3, list = TRUE, returnTrain = FALSE)
dat_1 = dat[ flds[[1]], ]
dat_2 = dat[ flds[[2]], ]
dat_3 = dat[ flds[[3]], ]
```
...
```{r echo=FALSE}
train = function(x, all_y_trues, pred_dat, learn_rate = 0.01, epochs = 40, plot = FALSE)
{
  # --- 梯度下降
...
```{r echo=FALSE}
dat = dat_original %>% 
  dplyr::select(star_rating_square, total_emotion)
#dat = scale(dat)
...
dat_correct = data.frame(
  训练方法 = c('logit_GD', 'logit_SGD', 'Neural Network', '综合法'),
