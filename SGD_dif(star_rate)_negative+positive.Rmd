...
dat = all %>% 
  dplyr::select(review_date, star_rating, group, product_parent) %>% 
...
dat_original %>%
  ggplot(aes(x = negative, y = increase)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm")

dat_original$positive = scale(dat_original$positive)
dat_original$negative = scale(dat_original$negative)
...
# --- prepare
dat = dat_original %>% 
  dplyr::select(positive, negative, success)
...
```{r echo=FALSE}
flds <- createFolds(dat$positive, k = 10, list = TRUE, returnTrain = FALSE)
dat_1 = dat[ flds[[1]], ]
dat_2 = dat[ flds[[2]], ]
dat_3 = dat[ flds[[3]], ]
```
